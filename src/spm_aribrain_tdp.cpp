 ...
 *
 *   [nclus,ixclus,tdpclus] = spm_aribrain_tdp(m,conn,alpha,gamma,simes,dims,maskI,indexp,ordp,rankp,allp,sortp);
 *
...
public:
    ...
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        std::vector<double> tdpclus;
        nclus.reserve(clusterlist.size());
        ixclus.reserve(m);
        tdpclus.reserve(clusterlist.size());
        for(std::list<std::vector<int>>::iterator it = clusterlist.begin(); it != clusterlist.end(); ++it)
        {
            ...
            tdpclus.push_back(tdps[(*it)[(*it).size()-1]]);
        }
        ...
        outputs[2] = factory.createArray({1,tdpclus.size()}, tdpclus.begin(), tdpclus.end());  // cluster TDP
    }
