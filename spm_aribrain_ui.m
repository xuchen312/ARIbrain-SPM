 function varargout = spm_aribrain_ui(varargin)
% User interface for SPM/PPM results: Display and analysis of regional effects
% FORMAT [hReg,xSPM,SPM] = spm_aribrain_ui('Setup',[xSPM])
%
...
%                  volume of interest. (Tabulation by spm_aribrain_list.m)
%                                            - see spm_VOI.m
%
...
%                           ----------------
%
% In addition to setting up the results section, spm_aribrain_ui.m sets
...
% $Id: spm_aribrain_ui.m 4661 2012-02-20 17:48:16Z guillaume $
...
% allowed fields.
%
% FORMAT hReg = spm_aribrain_ui('SetupGUI',M,DIM,xSPM,Finter)
...
%
% FORMAT spm_aribrain_ui('DrawButts',hReg,DIM,Finter,WS,FS)
% Draw GUI buttons
% hReg    - handle of XYZ registry object
% DIM     - 3 vector of image X, Y & Z dimensions
...
%
% FORMAT hFxyz = spm_aribrain_ui('DrawXYZgui',M,DIM,xSPM,xyz,Finter)
...
%
% FORMAT spm_aribrain_ui('EdWidCB')
% Callback for editable XYZ control widgets
%
% FORMAT spm_aribrain_ui('UpdateSPMval',hFxyz)
% FORMAT spm_aribrain_ui('UpdateSPMval',UD)
...
%
% FORMAT xyz = spm_aribrain_ui('GetCoords',hFxyz)
...
%
% FORMAT [xyz,d] = spm_aribrain_ui('SetCoords',xyz,hFxyz,hC)
...
% NB: When using the results section, should use XYZregistry to get/set location
%
% FORMAT hFxyz = spm_aribrain_ui('FindXYZframe',h)
...
%
% FORMAT hMP = spm_aribrain_ui('LaunchMP',M,DIM,hReg,hBmp)
% Prototype callback handler for integrating MultiPlanar toolbox
%
% FORMAT spm_aribrain_ui('Delete',h)
...
%==========================================================================
    ...
    hReg      = spm_aribrain_ui('SetupGUI',M,DIM,xSPM,Finter);
    ...
    %======================================================================
        ...
        %------------------------------------------------------------------
        hFxyz      = spm_aribrain_ui('DrawXYZgui',M,DIM,varargin{4},xyz,Finter);
        spm_XYZreg('XReg',hReg,hFxyz,'spm_aribrain_ui');
 
        %-Set up buttons for results functions
        %------------------------------------------------------------------
        spm_aribrain_ui('DrawButts',hReg,DIM,Finter,WS,FS);
    ...
    %======================================================================
        ...
        uicontrol(Finter,'Style','PushButton','String','whole brain','FontSize',FS(10),...
            ...
            'Callback','TabDat = spm_aribrain_list(''List'',xSPM,hReg);',...
        ...
        uicontrol(Finter,'Style','PushButton','String','current cluster','FontSize',FS(10),...
            ...
            'Callback','TabDat = spm_aribrain_list(''ListCluster'',xSPM,hReg);',...
            'Interruptible','on','Enable','on',...
        ...
        hClear = uicontrol(Finter,'Style','PushButton','String','clear',...
            ...
            'Callback',['spm_aribrain_ui(''Clear''); ',...
        ...
        hExit  = uicontrol(Finter,'Style','PushButton','String','exit',...
            ...
            'Callback','spm_aribrain_ui(''close'')',...
            'Interruptible','on','Enable','on',...
        ...
        hHelp  = uicontrol(Finter,'Style','PushButton','String','?',...
            ...
            'Callback','spm_help(''spm_aribrain_ui'')',...
            'Interruptible','on','Enable','on',...
    ...
    %======================================================================
        % hFxyz = spm_aribrain_ui('DrawXYZgui',M,DIM,xSPM,xyz,Finter)
        ...
        hX   = uicontrol(Finter,'Style','Edit','String',sprintf('%.2f',xyz(1)),...
            ...
            'Tag','hX',...
            'Callback','spm_aribrain_ui(''EdWidCB'')');
    ...
    %======================================================================
        ...
        if abs(xyz(d)-nxyz(d))>0
            ...
            spm_aribrain_ui('UpdateSPMval',UD)
        end
    ...
    %======================================================================
        % spm_aribrain_ui('UpdateSPMval',hFxyz)
        % spm_aribrain_ui('UpdateSPMval',UD)
    ...
    %======================================================================
        % xyz = spm_aribrain_ui('GetCoords',hFxyz)
        if nargin<2, hFxyz='Interactive'; else hFxyz=varargin{2}; end
        hFxyz     = spm_aribrain_ui('FindXYZframe',hFxyz);
    ...
    %======================================================================
        % [xyz,d] = spm_aribrain_ui('SetCoords',xyz,hFxyz,hC)
        if nargin<4, hC=0; else hC=varargin{4}; end
        if nargin<3, hFxyz=spm_aribrain_ui('FindXYZframe'); else hFxyz=varargin{3}; end
    ...
    %======================================================================
        % hFxyz = spm_aribrain_ui('FindXYZframe',h)
    ...
    %======================================================================
        ...
        h4 = uicontrol(Finter,'Style','PopUp',...
            ...
            'Callback','spm_aribrain_ui(''PlotUiCB'')',...
            'Interruptible','on','Enable','on',...
        ...
        set(hGraphUIbg,'UserData',...
            ...
            'DeleteFcn','spm_aribrain_ui(''Delete'',get(gcbo,''UserData''))')
    ...
    %======================================================================
        % Fgraph = spm_aribrain_ui('Clear',F,mode)
    ...
    %======================================================================
        % hMP = spm_aribrain_ui('LaunchMP',M,DIM,hReg,hBmp)
        ...
        set(hBmp,'DeleteFcn','spm_aribrain_ui(''delete'',get(gcbo,''UserData''))')
...
[hReg,xSPM,SPM] = spm_aribrain_ui('setup',xSPM2);
...
switch lower(action)
    ...
    case 'current'
        [xyzmm,i] = spm_XYZreg('NearestXYZ',...
            spm_aribrain_ui('GetCoords'),xSPM.XYZmm);
        spm_aribrain_ui('SetCoords',xSPM.XYZmm(:,i));
