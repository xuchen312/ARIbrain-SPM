function varargout = spm_aribrain_list(varargin)
% Display an analysis of SPM{.}
% FORMAT TabDat = spm_aribrain_list('List',xSPM,hReg,[Num,Dis,Str])
% Summary list of local maxima for entire volume of interest
% FORMAT TabDat = spm_aribrain_list('ListCluster',xSPM,hReg,[Num,Dis,Str])
...
%
% hReg   - Handle of results section XYZ registry (see spm_aribrain_ui.m)
%
...
%                           ----------------
%
% FORMAT spm_aribrain_list('TxtList',TabDat,c)
...
%                           ----------------
%
% FORMAT spm_aribrain_list('SetCoords',xyz,hAx,hReg)
...
%
% spm_aribrain_list characterizes SPMs (thresholded at u and k) in terms of
...
% $Id: spm_aribrain_list.m 6950 2016-11-25 12:05:08Z guillaume $
...
% FORMAT TabDat = spm_aribrain_list('List',xSPM,hReg,[Num,Dis,Str])
    ...
    TabDat = spm_aribrain_list('Table',xSPM,varargin{4:end});
    
    spm_aribrain_list('Display',TabDat,hReg);
...
case 'table'                                                        %-Table
%==========================================================================
    % FORMAT TabDat = spm_aribrain_list('table',xSPM,[Num,Dis,Str])
    ...
    %Tmap = spm_read_vols(spm_vol(fullfile(xSPM.swd,xSPM.Vspm.fname)));
    Tmap = spm_read_vols(spm_vol(xSPM.Vspm));
    % convert statistics to p-values
    switch STAT
        case 'F'
            Pmap = 1-spm_Fcdf(Tmap,df);
        case 'T'
            Pmap = spm_Tcdf(-abs(Tmap),df(2))*2;  % two-sided p-values
            %Pmap = 1-spm_Tcdf(Tmap,df(2));       % one-sided p-values
        case 'X'
            Pmap = spm_Xcdf(Tmap,df(2),'upper');
        case 'Z'
    ...
    if isempty(xSPM.Im)
        ...
        mask = split(xSPM.Im,',');
        mask = spm_read_vols(spm_vol(mask{1}));
        mask = mask~=0;
    end
    if ~isempty(xSPM.Ex) && xSPM.Ex > 0
        mask = 1 - mask;
    end
    mask = int32(mask);
    % apply mask and extract all in-mask p-values
    allp  = Pmap(mask~=0);
    % sort p-values in ascending order
    [sortp,ordp] = sort(allp);

    %-Prepare optional inputs passed on to the function
    %----------------------------------------------------------------------
    varargins = cell(8,1);
    iv = 1;
    if isfield(xSPM,'alpha')
        varargins{iv}   = 'alpha';
    ...
    if isfield(xSPM,'tdpth')
        ...
        % update xSPM with the new cluster info
        xSPM.Z         = Z;
        xSPM.XYZ       = XYZ;
        xSPM.XYZmm     = XYZmm;
        xSPM.thresDesc = ['TDP>=' num2str(xSPM.tdpth)];
    end
    ...
    while numel(find(isfinite(Z)))
        ...
        if STAT ~= 'P'
            ...
            if isfield(xSPM,'tdpth')
                ...
                %--- For TDP-based cluster i,
                tdp = tdpclus(Lclus(L{A(i)}(1,1),L{A(i)}(2,1),L{A(i)}(3,1)));

            else

                %--- For RFT-based cluster i,
                %--- region A(i) has N(i) voxels, whose positions are in L{i}.
                %--- regions are sorted in descending order of max(T).
                % N(i)     - cluster size
                % Z(i)/U   - value of the ith maximum
                % XYZ(:,i) - location of the ith maximum ([X Y Z] coordinates)
                % A(i)     - region number (c=max(A))
                % L{i}     - cell array of voxel locations for cluster i
                %            (3 x N(i) matrix)

                % set voxels within cluster i to be -1 in the mask
                iL = L{A(i)};
                for v = 1:N(i)
                    mask(iL(1,v),iL(2,v),iL(3,v)) = -1;
                end
                ixp = mask(mask~=0);
                ixp = find(ixp<0);
                % compute TDP for RFT-based clustering
                tdp = spm_aribrain_clusterTDP(allp,sortp,{ixp},varargins{:});
                % set mask back to be >=0
                for v = 1:N(i)
                    mask(iL(1,v),iL(2,v),iL(3,v)) = 1;
                end
    ...
    %======================================================================
    % FORMAT spm_aribrain_list('display',TabDat,hReg)
    ...
    %----------------------------------------------------------------------
    xyzmm = spm_aribrain_ui('GetCoords');
    ...
    uimenu(h,'Label','Print text table',...
        'CallBack',...
        'spm_aribrain_list(''txtlist'',get(get(gcbo,''Parent''),''UserData''),3)',...
    ...
    if ispc
        ...
        'CallBack',...
        'spm_aribrain_list(''xlslist'',get(get(gcbo,''Parent''),''UserData''));',...
        'Interruptible','off','BusyAction','Cancel');
    end
    uimenu(h,'Label','Export to CSV file',...
        'CallBack',...
        'spm_aribrain_list(''csvlist'',get(get(gcbo,''Parent''),''UserData''));',...
        'Interruptible','off','BusyAction','Cancel');
...
%         'Interruptible','off','BusyAction','Cancel');
    ...
    spm_XYZreg('Add2Reg',hReg,hAx,'spm_aribrain_list');
    ...
    % FORMAT TabDat = spm_aribrain_list('ListCluster',xSPM,hReg,[Num,Dis,Str])
        ...
        %------------------------------------------------------------------
        varargout = { spm_aribrain_list('list',xSPM,hReg,Num,Dis,Str) };
    ...
    %======================================================================
    % FORMAT spm_aribrain_list('XLSList',TabDat,ofile)
    ...
    %======================================================================
    % FORMAT spm_aribrain_list('CSVList',TabDat,ofile)
    ...
    %======================================================================
    % FORMAT spm_aribrain_list('label',atlas)
